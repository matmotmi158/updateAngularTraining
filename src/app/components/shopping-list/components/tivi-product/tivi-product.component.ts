...
@Component({
  selector: 'app-tivi-product',
  templateUrl: './tivi-product.component.html',
  styleUrls: ['./tivi-product.component.css']
})
export class TiviProductComponent implements OnInit,OnDestroy {
  listItem !: Product[]
  ...
  getProduct(){
    this.subscription = this.productService.getAllProduct().subscribe(data=>{
      this.listItem = data
      this.listItem = this.listItem.filter(x=>x.title.toLowerCase() === "tivi")
    })
  }
