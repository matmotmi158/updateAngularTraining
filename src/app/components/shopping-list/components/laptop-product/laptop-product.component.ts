...
@Component({
  selector: 'app-laptop-product',
  templateUrl: './laptop-product.component.html',
  styleUrls: ['./laptop-product.component.css']
})
export class LaptopProductComponent implements OnInit,OnDestroy {
  listItem !: Product[]
  ...
  getProduct(){
    this.subscription = this.productService.getAllProduct().subscribe(data=>{
      this.listItem = data
      this.listItem = this.listItem.filter(x=>x.title.toLowerCase() === "laptop")
    })
  }
